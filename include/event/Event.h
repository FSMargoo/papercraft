...
using PDelegate = std::function<void(Parameters...)>;
...
/// 
template <class... Parameters>
class PFunctionConnection : public PConnectUnit<Parameters...> {
public:
	/**
...
class PClassConnection : public PConnectUnit<Parameter...> {
public:
	/// 
...
public:
	...
	inline void Connect(ObjectType* Object, ClassFunctionPointer<ObjectType> Function) {
		...
		if (!Operation(Object, Function, PEventOperation::Lookup)) {
			std::shared_ptr<PConnectUnit<Parameters...> > pointer(new PClassConnection<ObjectType, Parameters...>(Object, Function));
			_slots->emplace_back(pointer);
		}
	}
	/// 

...
#define OPEN_STRING_CODE_BLOCK(code)                                                                                   \
	do {                                                                                                               \
		code                                                                                                           \
	} while (false)
#endif
	...
	OPEN_STRING_CODE_BLOCK({                                                                                           \
		if (OPEN_STRING_UNLIKELY(!(expression))) {                                                                     \
			OPEN_STRING_PRINT_FORMATTED_DEBUG_MESSAGE(                                                                 \
				"\nCheck failed: {}\n\t[" __FILE__                                                                     \
				":" OPEN_STRING_STRINGIFY_EXPANDED(__LINE__) "] " OPEN_STRING_STRINGIFY(expression) "\n",              \
				__VA_ARGS__);                                                                                          \
...
#define OPEN_STRING_RESULT_CHECK(result)                                                                               \
	OPEN_STRING_CODE_BLOCK(HRESULT result__{result}; OPEN_STRING_CHECK(SUCCEEDED(result__), "Result check failed! {}", \
																	   _com_error{result__}.ErrorMessage());)
#endif
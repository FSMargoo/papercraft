...
namespace ostr {
template <class T, u64 I = 0> struct OPEN_STRING_API sequence {
public:
	sequence() noexcept;

	template <class T1, u64 I1> explicit sequence(const std::array<T1, I1> &array) noexcept;

	template <class T1, u64 I1> explicit sequence(std::array<T1, I1> &&array) noexcept;
	...
	template <class... Args> void resize(const u64 size, Args &&...args) noexcept;

	void push_back(T element) noexcept;

	template <class... Args> void emplace_back(Args &&...args) noexcept;

	template <class T1 = T> T1 pop_back() noexcept;
...
template <class T, u64 I> sequence<T, I>::sequence() noexcept = default;
...
template <class T, u64 I> template <class T1, u64 I1> sequence<T, I>::sequence(std::array<T1, I1> &&array) noexcept {
	this->reserve(array.size());
	std::move(array.data(), array.end(), this->data());
	this->size_ = array.size();
}

template <class T, u64 I> sequence<T, I>::sequence(std::initializer_list<T> list) noexcept {
...
sequence<T, I>::sequence(sequence &&other) noexcept
	: capacity_{other.capacity_}, size_{other.size_}, storage_{std::move(other.storage_)} {
	other.capacity_ = ELEMENT_SHORT_CAPACITY;
	other.size_		= 0;
}

template <class T, u64 I> sequence<T, I>::~sequence() noexcept {
...
template <class T, u64 I> typename sequence<T, I>::const_iterator sequence<T, I>::cbegin() const noexcept {
	return const_iterator{this->data()};
}

template <class T, u64 I> typename sequence<T, I>::const_iterator sequence<T, I>::cend() const noexcept {
	return const_iterator{this->data_at(this->size_)};
}

template <class T, u64 I> typename sequence<T, I>::iterator sequence<T, I>::begin() noexcept {
	return iterator{this->data()};
}

template <class T, u64 I> typename sequence<T, I>::iterator sequence<T, I>::end() noexcept {
	return iterator{this->data_at(this->size_)};
}

template <class T, u64 I> typename sequence<T, I>::const_iterator sequence<T, I>::begin() const noexcept {
	return this->cbegin();
}

template <class T, u64 I> typename sequence<T, I>::const_iterator sequence<T, I>::end() const noexcept {
	return this->cend();
}

template <class T, u64 I> bool sequence<T, I>::is_empty() const noexcept {
	return this->size_ == 0;
}

template <class T, u64 I> bool sequence<T, I>::operator==(const sequence &other) const noexcept {
...
template <class T, u64 I> u64 sequence<T, I>::size() const noexcept {
	return this->size_;
}

template <class T, u64 I> u64 sequence<T, I>::capacity() const noexcept {
	return this->capacity_;
}

template <class T, u64 I> T *sequence<T, I>::data() noexcept {
	return this->is_short() ? this->as_small().stack_storage.data() : this->as_large().heap_storage;
}

template <class T, u64 I> const T *sequence<T, I>::data() const noexcept {
	return this->is_short() ? this->as_small().stack_storage.data() : this->as_large().heap_storage;
}

template <class T, u64 I> void sequence<T, I>::empty() noexcept {
	for (T &e : *this)
		std::destroy_at(&e);
	this->size_ = 0;
}

template <class T, u64 I> void sequence<T, I>::empty(u64 size) noexcept {
	this->empty();
...
template <class T, u64 I> const T &sequence<T, I>::read_at(const u64 index) const noexcept {
	OPEN_STRING_CHECK(index < this->size_, "index out of range");
	return *(this->data_at(index));
}

template <class T, u64 I> const T &sequence<T, I>::read_from_last(const u64 index) const noexcept {
	OPEN_STRING_CHECK(index < this->size_, "index out of range");
	return this->read_at(this->size_ - index - 1);
}

template <class T, u64 I> T &sequence<T, I>::access_at(const u64 index) noexcept {
	OPEN_STRING_CHECK(index < this->size_, "index out of range");
	return *(this->data_at(index));
}

template <class T, u64 I> T &sequence<T, I>::access_from_last(const u64 index) noexcept {
	OPEN_STRING_CHECK(index < this->size_, "index out of range");
	return this->access_at(this->size_ - index - 1);
}

template <class T, u64 I> void sequence<T, I>::reserve(const u64 size) noexcept {
	if (size <= this->capacity_)
		return;
...
template <class T, u64 I> void sequence<T, I>::push_back_uninitialized(const u64 size) noexcept {
	...
	if (size_required > this->capacity_) {
		...
		u64 grown_capacity =
			round_ceil(static_cast<f64>(maximum({this->capacity_, 1})) * global_constant::GOLDEN_RATIO);
...
template <class T, u64 I> void sequence<T, I>::resize_uninitialized(const u64 size) noexcept {
...
template <class T, u64 I> void sequence<T, I>::push_back(T element) noexcept {
	this->emplace_back(std::move(element));
}

template <class T, u64 I> template <class... Args> void sequence<T, I>::emplace_back(Args &&...args) noexcept {
...
template <class T, u64 I> template <class T1> T1 sequence<T, I>::pop_back() noexcept {
...
template <class T, u64 I> void sequence<T, I>::append(const T *const source, const u64 size) noexcept {
	this->push_back_uninitialized(size);
	std::copy_n(source, size, this->data_at(this->size_ - size));
}

template <class T, u64 I> bool sequence<T, I>::is_short() const noexcept {
	return this->capacity_ == ELEMENT_SHORT_CAPACITY;
}

template <class T, u64 I> T *sequence<T, I>::data_at(u64 index) noexcept {
	return this->data() + index;
}

template <class T, u64 I> const T *sequence<T, I>::data_at(u64 index) const noexcept {
	return this->data() + index;
}
} // namespace ostr

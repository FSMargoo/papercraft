...
public:
	...
	constexpr text_view(const char *data, const u64 count) noexcept : view_{data, count} {
	}
	constexpr text_view(const char *str) noexcept : view_{str} {
	}
	constexpr text_view(codeunit_sequence_view view) noexcept : view_{std::move(view)} {
	}
	constexpr text_view(const codepoint &cp) noexcept : view_{cp.raw(), cp.size()} {
	}
	...
	struct const_iterator {
		...
		explicit constexpr const_iterator(codeunit_sequence_view::const_iterator v) noexcept : value(v) {
		}
		...
		constexpr const_iterator &operator+=(const i64 diff) noexcept {
			if (diff != 0) {
				...
				const auto &op		 = diff > 0 ? static_cast<self_operator>(&const_iterator::operator++)
												: static_cast<self_operator>(&const_iterator::operator--);
	...
	[[nodiscard]] constexpr codepoint read_at(const u64 index) const noexcept {
		const char *data = &this->view_.read_at(this->get_codeunit_index(index));
	...
	[[nodiscard]] constexpr u64 index_of(const text_view &pattern, const u64 from = 0,
	...
	[[nodiscard]] constexpr u64 index_of(const codepoint &pattern, const u64 from = 0,
										 const u64 size = SIZE_MAX) const noexcept {
		return this->index_of(text_view{pattern}, from, size);
	}

	[[nodiscard]] constexpr u64 last_index_of(const text_view &pattern, const u64 from = 0,
											  const u64 size = SIZE_MAX) const noexcept {
		if (pattern.is_empty())
			return global_constant::INDEX_INVALID;
		u64 raw_from = from;
...
[[nodiscard]] constexpr ostr::text_view operator""_txtv(const char *str, const size_t len) noexcept {
	return {str, len};
}
} // namespace literal

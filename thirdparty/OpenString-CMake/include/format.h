...
template <class T, typename = void> struct argument_formatter {
	static codeunit_sequence produce(const T &value, const codeunit_sequence_view &specification) {
		...
		for (u64 i = 0; i < size; ++i) {
			...
			raw += details::format_integer(static_cast<u64>(memory), "02x"_cuqv);
		}

		if (specification == "r"_cuqv) // output raw memory bytes
			return raw;

		OPEN_STRING_CHECK(false, "Undefined format with raw memory bytes:{}!", raw);
		return {};
	}
};
...
public:
	...
	struct const_iterator {
		...
		explicit constexpr const_iterator(codeunit_sequence_view v) noexcept : format_mold(std::move(v)) {
		}
		...
		[[nodiscard]] constexpr result_parse_run parse_run(const u64 from_index) const {
			const u64 index = this->format_mold.index_of_any("{}"_cuqv, from_index);
			...
			OPEN_STRING_CHECK(format_mold.read_at(index) != '}', "Unclosed right brace is not allowed!");
			const u64 index_next = this->format_mold.index_of_any("{}"_cuqv, index + 1);
			OPEN_STRING_CHECK(index_next != global_constant::INDEX_INVALID && format_mold.read_at(index_next) != '{',
							  "Unclosed left brace is not allowed!");
...
struct argument_value_package {
	using argument_value_formatter_type = codeunit_sequence (*)(const void			  *value,
																codeunit_sequence_view specification);
	...
	explicit constexpr argument_value_package(const T &v) noexcept
		: value(static_cast<const void *>(&v)), argument_value_formatter(format_argument_value<T>) {
	}
...
template <class... Args> codeunit_sequence produce_format(const format_mold_view &format_mold, const Args &...args) {
	...
	for (const auto [type, run] : format_mold) {
		...
		case format_mold_view::run_type::formatter: {
			const auto [index_run, specification] = run.subview(1, run.size() - 2).split(":"_cuqv);
			u64 current_index					  = next_index;
			if (index_run.is_empty()) {
				OPEN_STRING_CHECK(index_type != indexing_type::manual,
								  "Manual index is not allowed mixing with automatic index!");
				index_type = indexing_type::automatic;
				++next_index;
			} else {
				OPEN_STRING_CHECK(index_type != indexing_type::automatic,
								  "Automatic index is not allowed mixing with manual index!");
				index_type = indexing_type::manual;
				[[maybe_unused]] const auto [last, error] =
					std::from_chars(index_run.data(), index_run.cend().data(), current_index);
				OPEN_STRING_CHECK(last == index_run.cend().data(), "Invalid format index [{}]!", index_run);
			}
			OPEN_STRING_CHECK(current_index < argument_count,
							  "Invalid format index [{}]: Index should be less than count of argument [{}]!",
							  current_index, argument_count);
...
template <class T> struct argument_formatter<T, std::enable_if_t<std::is_integral_v<T> && std::is_signed_v<T>>> {
	static codeunit_sequence produce(const T &value, const codeunit_sequence_view &specification) {
		return details::format_integer(static_cast<i64>(value), specification);
	}
};

template <class T> struct argument_formatter<T, std::enable_if_t<std::is_integral_v<T> && std::is_unsigned_v<T>>> {
	static codeunit_sequence produce(const T &value, const codeunit_sequence_view &specification) {
		return details::format_integer(static_cast<u64>(value), specification);
	}
};

template <class T> struct argument_formatter<T, std::enable_if_t<std::is_floating_point_v<T>>> {
	static codeunit_sequence produce(const T &value, const codeunit_sequence_view &specification) {
		return details::format_float(static_cast<f64>(value), specification);
	}
};

template <> struct argument_formatter<const char *> {
	static codeunit_sequence produce(const char *value, const codeunit_sequence_view &specification) {
		return codeunit_sequence{value};
	}
};

template <size_t N> struct argument_formatter<char[N]> {
	static codeunit_sequence produce(const char (&value)[N], const codeunit_sequence_view &specification) {
		return codeunit_sequence{value};
	}
};

template <> struct argument_formatter<codeunit_sequence_view> {
	static codeunit_sequence produce(const codeunit_sequence_view &value, const codeunit_sequence_view &specification) {
		return codeunit_sequence{value};
	}
};

template <> struct argument_formatter<codeunit_sequence> {
	static const codeunit_sequence &produce(const codeunit_sequence		 &value,
											const codeunit_sequence_view &specification) {
		return value;
	}
};

template <> struct argument_formatter<std::nullptr_t> {
...
template <class T> struct argument_formatter<T *> {
	static codeunit_sequence produce(const T *value, const codeunit_sequence_view &specification) {

...
[[nodiscard]] constexpr char get_utf8_mask(const u64 length) noexcept {
	constexpr char masks[] = {
		0b00111111_as_char, 0b01111111_as_char, 0b00011111_as_char, 0b00001111_as_char, 0b00000111_as_char,
	};
	return masks[length];
}
...
[[nodiscard]] constexpr std::array<char, UTF8_SEQUENCE_MAXIMUM_LENGTH> utf32_to_utf8(const char32_t utf32) noexcept {
	if (utf32 <= get_utf8_maximum_codepoint(1))
		return {static_cast<char>(utf32), 0, 0, 0};
	if (utf32 <= get_utf8_maximum_codepoint(2))
		return {static_cast<char>((utf32 >> 6) | 0xc0), static_cast<char>((utf32 & 0x3f) | 0x80), 0, 0};
	if (utf32 <= get_utf8_maximum_codepoint(3))
		return {static_cast<char>((utf32 >> 12) | 0xe0), static_cast<char>(((utf32 >> 6) & 0x3f) | 0x80),
				static_cast<char>((utf32 & 0x3f) | 0x80), 0};
	return {static_cast<char>((utf32 >> 18) | 0xf0), static_cast<char>(((utf32 >> 12) & 0x3f) | 0x80),
			static_cast<char>(((utf32 >> 6) & 0x3f) | 0x80), static_cast<char>((utf32 & 0x3f) | 0x80)};
}
...
[[nodiscard]] constexpr std::array<char16_t, utf16::SEQUENCE_MAXIMUM_LENGTH> utf32_to_utf16(
	char32_t const utf32) noexcept {
...
[[nodiscard]] constexpr std::array<char16_t, utf16::SEQUENCE_MAXIMUM_LENGTH> utf8_to_utf16(char const *const utf8,
																						   const u64 length) noexcept {
	return utf32_to_utf16(utf8_to_utf32(utf8, length));
}

[[nodiscard]] constexpr std::array<char, UTF8_SEQUENCE_MAXIMUM_LENGTH> utf16_to_utf8(char16_t const *const utf16,
																					 const u64 length) noexcept {
...
struct codepoint {
	...
	explicit constexpr codepoint(const char *const v, const u8 length) noexcept : sequence{} {
		for (u8 i = 0; i < length; ++i)
			sequence[i] = v[i];
	}

	explicit constexpr codepoint(const char *v) noexcept : codepoint{v, unicode::parse_utf8_length(v[0])} {
	}
	...
	explicit constexpr codepoint(const char32_t cp) noexcept : sequence{unicode::utf32_to_utf8(cp)} {
	}

	struct const_iterator {
		constexpr const_iterator() noexcept = default;

		explicit constexpr const_iterator(const char *v) noexcept : value{v} {
		}
	...
	std::array<char, unicode::UTF8_SEQUENCE_MAXIMUM_LENGTH> sequence;
};
...
[[nodiscard]] constexpr ostr::codepoint operator""_cp(const char c) noexcept {

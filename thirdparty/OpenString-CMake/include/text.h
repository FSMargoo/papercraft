...
public:
	...
	static text from_utf8(const char *string_utf8) noexcept;
	...
	template <class... Args> static text	  build(const Args &...argument);
	template <typename Container> static text join(const Container &container, const text_view &separator) noexcept;
	...
	text &append(char codeunit, u64 count = 1) noexcept;
	...
	[[nodiscard]] u64 index_of(const text_view &pattern, u64 from = 0,
							   u64 size = global_constant::SIZE_INVALID) const noexcept;
	[[nodiscard]] u64 last_index_of(const text_view &pattern, u64 from = 0,
									u64 size = global_constant::SIZE_INVALID) const noexcept;

	[[nodiscard]] u64 count(const text_view &pattern, u64 from = 0,
							u64 size = global_constant::SIZE_INVALID) const noexcept;
...
namespace details {
template <> [[nodiscard]] constexpr codeunit_sequence_view view_sequence<text_view>(const text_view &v) {
	return v.raw();
}
template <> [[nodiscard]] inline codeunit_sequence_view view_sequence<text>(const text &v) {
	return v.view().raw();
}
} // namespace details

template <class... Args> text text::build(const Args &...argument) {
	return {codeunit_sequence::build(argument...)};
}

template <typename Container> text text::join(const Container &container, const text_view &separator) noexcept {
	return {codeunit_sequence::join(container, separator.raw())};
}

OPEN_STRING_API [[nodiscard]] bool operator==(const text_view &lhs, const text &rhs) noexcept;

template <> struct argument_formatter<text_view> {
...
template <> struct argument_formatter<text> {

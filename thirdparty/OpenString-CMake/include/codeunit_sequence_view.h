...
public:
	...
	constexpr codeunit_sequence_view(const char *data, const u64 count) noexcept;
	...
	[[nodiscard]] constexpr const char &read_at(const u64 index) const noexcept;
	[[nodiscard]] constexpr const char &read_from_last(const u64 index) const noexcept;
	...
	[[nodiscard]] constexpr u64 index_of(const char codeunit, const u64 from = 0,
										 const u64 size = SIZE_MAX) const noexcept;
	[[nodiscard]] constexpr u64 last_index_of(const codeunit_sequence_view &pattern, const u64 from = 0,
											  const u64 size = SIZE_MAX) const noexcept;
	[[nodiscard]] constexpr u64 index_of_any(const codeunit_sequence_view &units, const u64 from = 0,
											 const u64 size = SIZE_MAX) const noexcept;
	[[nodiscard]] constexpr u64 last_index_of_any(const codeunit_sequence_view &units, const u64 from = 0,
												  const u64 size = SIZE_MAX) const noexcept;
	...
	[[nodiscard]] constexpr std::array<codeunit_sequence_view, 2> split(
		const codeunit_sequence_view &splitter) const noexcept;
	u32 split(const codeunit_sequence_view &splitter, std::vector<codeunit_sequence_view> &pieces,
			  bool cull_empty = true) const noexcept;

	[[nodiscard]] constexpr u64 count(const codeunit_sequence_view &pattern, u64 from = 0,
									  u64 size = global_constant::SIZE_INVALID) const noexcept;
	...
	[[nodiscard]] constexpr codeunit_sequence_view trim_start(
		const codeunit_sequence_view &units = codeunit_sequence_view(" \t")) const noexcept;
	[[nodiscard]] constexpr codeunit_sequence_view trim_end(
		const codeunit_sequence_view &units = codeunit_sequence_view(" \t")) const noexcept;
	[[nodiscard]] constexpr codeunit_sequence_view trim(
		const codeunit_sequence_view &units = codeunit_sequence_view(" \t")) const noexcept;
...
namespace details {
[[nodiscard]] constexpr u64 get_sequence_length(const char *str) noexcept {
	if (!str)
		return 0;
	u64 count = 0;
	while (str[count] != 0)
...
constexpr codeunit_sequence_view &codeunit_sequence_view::operator=(const codeunit_sequence_view &other) noexcept =
	default;
constexpr codeunit_sequence_view &codeunit_sequence_view::operator=(codeunit_sequence_view &&other) noexcept = default;

constexpr codeunit_sequence_view::codeunit_sequence_view(const char *data, const u64 count) noexcept
	: size_{count}, data_{data} {
}

constexpr codeunit_sequence_view::codeunit_sequence_view(const char *data, const char *last) noexcept
	: size_{static_cast<u64>(last - data)}, data_{data} {
}

constexpr codeunit_sequence_view::codeunit_sequence_view(const char *str) noexcept
	: codeunit_sequence_view{str, details::get_sequence_length(str)} {
}

constexpr codeunit_sequence_view::codeunit_sequence_view(const char &c) noexcept : codeunit_sequence_view{&c, 1} {
}
...
constexpr codeunit_sequence_view::const_iterator::const_iterator(const char *v) noexcept : value{v} {
}

constexpr const char *codeunit_sequence_view::const_iterator::data() const noexcept {
	return this->value;
}

constexpr const char &codeunit_sequence_view::const_iterator::operator*() const noexcept {
	return *this->value;
}
...
constexpr codeunit_sequence_view::const_iterator &codeunit_sequence_view::const_iterator::operator+=(
	const i64 diff) noexcept {
	this->value += diff;
	return *this;
}
...
constexpr bool codeunit_sequence_view::operator==(const char *rhs) const noexcept {
...
inline const char *codeunit_sequence_view::data() const noexcept {
	return this->data_;
}
...
constexpr codeunit_sequence_view codeunit_sequence_view::subview(const u64 from, const u64 size) const noexcept {
	...
	const char *first_data	= this->data_ + from;
	const u64	actual_size = minimum(size, self_size - from);
	return {first_data, actual_size};
}

constexpr const char &codeunit_sequence_view::read_at(const u64 index) const noexcept {
	return this->data_[index];
}

constexpr const char &codeunit_sequence_view::read_from_last(const u64 index) const noexcept {
	return this->data_[this->size_ - index - 1];
}

constexpr u64 codeunit_sequence_view::index_of(const codeunit_sequence_view &pattern, const u64 from,
	...
	const char pattern_last = pattern.read_from_last(0);
	u64		   skip			= 1;
...
constexpr u64 codeunit_sequence_view::index_of(const char codeunit, const u64 from, const u64 size) const noexcept {
	if (codeunit == 0)
		return global_constant::INDEX_INVALID;
	const codeunit_sequence_view view = this->subview(from, size);
...
constexpr u64 codeunit_sequence_view::last_index_of(const codeunit_sequence_view &pattern, const u64 from,
													const u64 size) const noexcept {
	const codeunit_sequence_view view = this->subview(from, size);
	if (pattern.is_empty())
		return global_constant::INDEX_INVALID;
	...
	const char pattern_first = pattern.read_at(0);
	u64		   skip			 = 1;
...
constexpr u64 codeunit_sequence_view::index_of_any(const codeunit_sequence_view &units, const u64 from,
												   const u64 size) const noexcept {
	const u64 actual_size = minimum(this->size() - from, size);
	const u64 final_index = from + actual_size;
	for (u64 i = from; i < final_index; ++i)
		if (const char &unit = this->read_at(i); units.contains(unit))
			return i;
	return global_constant::INDEX_INVALID;
}

constexpr u64 codeunit_sequence_view::last_index_of_any(const codeunit_sequence_view &units, const u64 from,
														const u64 size) const noexcept {
	const u64 actual_size = minimum(this->size() - from, size);
	const u64 final_index = from + actual_size;
...
constexpr bool codeunit_sequence_view::contains(const char codeunit) const noexcept {
	return this->index_of(codeunit) != global_constant::INDEX_INVALID;
}

constexpr std::array<codeunit_sequence_view, 2> codeunit_sequence_view::split(
	const codeunit_sequence_view &splitter) const noexcept {
...
inline u32 codeunit_sequence_view::split(const codeunit_sequence_view		 &splitter,
										 std::vector<codeunit_sequence_view> &pieces,
										 const bool							  cull_empty) const noexcept {
...
constexpr u64 codeunit_sequence_view::count(const codeunit_sequence_view &pattern, const u64 from,
											const u64 size) const noexcept {
...
constexpr codeunit_sequence_view codeunit_sequence_view::remove_prefix(
	const codeunit_sequence_view &prefix) const noexcept {
	return this->starts_with(prefix) ? this->subview(prefix.size()) : *this;
}

constexpr codeunit_sequence_view codeunit_sequence_view::remove_suffix(
	const codeunit_sequence_view &suffix) const noexcept {
	return this->ends_with(suffix) ? this->subview(0, this->size() - suffix.size()) : *this;
}

constexpr codeunit_sequence_view codeunit_sequence_view::trim_start(
	const codeunit_sequence_view &units) const noexcept {
	if (this->is_empty())
		return {};
	for (u64 i = 0; i < this->size(); ++i)
		if (const char codeunit = this->read_at(i); !units.contains(codeunit))
...
constexpr codeunit_sequence_view codeunit_sequence_view::trim_end(const codeunit_sequence_view &units) const noexcept {
	...
	for (u64 i = this->size(); i > 0; --i)
		if (const char codeunit = this->read_at(i - 1); !units.contains(codeunit))
...
[[nodiscard]] constexpr ostr::codeunit_sequence_view operator""_cuqv(const char *str, const size_t len) noexcept {
	return {str, len};
}
} // namespace literal

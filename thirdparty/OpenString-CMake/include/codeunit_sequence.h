...
public:
	...
	template <class... Args> static codeunit_sequence build(const Args &...argument);
	...
	[[nodiscard]] u64 last_index_of(const codeunit_sequence_view &pattern, u64 from = 0,
									u64 size = SIZE_MAX) const noexcept;
	...
	codeunit_sequence &replace(const codeunit_sequence_view &destination, const codeunit_sequence_view &source,
							   u64 from = 0, u64 size = SIZE_MAX);
	...
	codeunit_sequence &self_trim_start(
		const codeunit_sequence_view &characters = codeunit_sequence_view(" \t")) noexcept;
	...
	[[nodiscard]] codeunit_sequence_view view_trim_start(
		const codeunit_sequence_view &characters = codeunit_sequence_view(" \t")) const noexcept;
	[[nodiscard]] codeunit_sequence_view view_trim_end(
		const codeunit_sequence_view &characters = codeunit_sequence_view(" \t")) const noexcept;
	[[nodiscard]] codeunit_sequence_view view_trim(
		const codeunit_sequence_view &characters = codeunit_sequence_view(" \t")) const noexcept;
...
namespace details {
template <class T> [[nodiscard]] constexpr codeunit_sequence_view view_sequence(const T &v) {
	return codeunit_sequence_view{v};
}

template <> [[nodiscard]] inline codeunit_sequence_view view_sequence<codeunit_sequence>(const codeunit_sequence &v) {
	return v.view();
}
} // namespace details

template <class... Args> codeunit_sequence codeunit_sequence::build(const Args &...argument) {
	u64													size = 0;
...
template <typename Container>
codeunit_sequence codeunit_sequence::join(const Container			   &container,
										  const codeunit_sequence_view &separator) noexcept {

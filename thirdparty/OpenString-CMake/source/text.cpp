...
text::text(const char *str) noexcept : sequence_{str} {
}

text::text(const text_view &view) noexcept : sequence_{view.raw()} {
}

text::text(codeunit_sequence sequence) noexcept : sequence_{std::move(sequence)} {
}

text::text(const codeunit_sequence_view &sequence) noexcept : sequence_{sequence} {
}

text text::from_utf8(const char *string_utf8) noexcept {
...
text::iterator::iterator(text &t, const u64 from, const u64 size) noexcept : from{from}, size{size}, owner{&t} {
}

text::iterator::codepoint_accessor::codepoint_accessor(iterator &iter) noexcept : it_{iter} {
}
...
text::iterator &text::iterator::operator++() noexcept {
	...
	const char c		  = this->owner->sequence_.read_at(next_start);
...
text &text::append(const char codeunit, const u64 count) noexcept {

...
codeunit_sequence::iterator::iterator() noexcept : value{} {
}

codeunit_sequence::iterator::iterator(char *v) noexcept : value{v} {
}
...
codeunit_sequence::codeunit_sequence(const codeunit_sequence &other) noexcept : codeunit_sequence{other.view()} {
}

codeunit_sequence::codeunit_sequence(codeunit_sequence &&other) noexcept : store_{other.store_} {
	other.store_.fill(0);
}
...
codeunit_sequence::codeunit_sequence(const char *data) noexcept : codeunit_sequence(codeunit_sequence_view(data)) {
}
...
codeunit_sequence::codeunit_sequence(const char *data, const u64 count) noexcept
	: codeunit_sequence(codeunit_sequence_view(data, count)) {
}

codeunit_sequence::codeunit_sequence(const codeunit_sequence_view &sv) noexcept : codeunit_sequence(sv.size()) {
...
codeunit_sequence &codeunit_sequence::append(const char codeunit, const u64 count) noexcept {
	if (count <= 0)
		return *this;
...
codeunit_sequence &codeunit_sequence::replace(const codeunit_sequence_view &destination,
											  const codeunit_sequence_view &source, const u64 from, const u64 size) {
	if (source.is_empty())
		return *this;
...
codeunit_sequence &codeunit_sequence::replace(const codeunit_sequence_view &destination, const u64 from,
											  const u64 size) {
	const u64 self_size = this->size();
	...
	if (delta <= 0) {
		...
		if (delta != 0) {
			char	   *target = this->data() + from + destination.size();
			const char *source = target - delta;
...
u64 codeunit_sequence::last_index_of(const codeunit_sequence_view &pattern, const u64 from,
...
codeunit_sequence &codeunit_sequence::write_at(const u64 index, const char codeunit) noexcept {
	this->data()[index] = codeunit;
	return *this;
}

const char &codeunit_sequence::read_at(const u64 index) const noexcept {
	return this->view().read_at(index);
}

char &codeunit_sequence::operator[](const u64 index) noexcept {
	return this->data()[index];
}

const char &codeunit_sequence::operator[](const u64 index) const noexcept {
...
codeunit_sequence &codeunit_sequence::reverse(const u64 from, const u64 size) noexcept {
	...
	for (u64 i = 0; i < actual_size / 2; ++i) {
		const char temp = this->read_at(from + i);
...
u32 codeunit_sequence::split(const codeunit_sequence_view &splitter, std::vector<codeunit_sequence_view> &pieces,
...
char *codeunit_sequence::data() noexcept {
	return this->is_short() ? this->as_sso().data.data() : this->as_norm().data;
}

const char *codeunit_sequence::data() const noexcept {
	return this->is_short() ? this->as_sso().data.data() : this->as_norm().data;
}

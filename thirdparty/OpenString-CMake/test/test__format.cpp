...
TEST(format, undefined_type) {
	...
	EXPECT_CHECKED_WITH_MESSAGE(format("{}"_cuqv, ts1),
								"Undefined format with raw memory bytes: 40 e2 01 00 f1 fb 09 00!");
...
TEST(format, manual_index) {
	...
	EXPECT_EQ("My name is 繁星明 and I'm 25 years old."_cuqv,
			  format("My name is {1} and I'm {0} years old."_cuqv, 25, "繁星明"_cuqv));
}
...
TEST(format, format_exception) {
	...
	EXPECT_CHECKED_WITH_MESSAGE(format("{}{}"_cuqv, 3),
								"Invalid format index [1]: Index should be less than count of argument [1]!");
	...
	EXPECT_CHECKED_WITH_MESSAGE(format("{abc}"_cuqv, 123),
								"Invalid format index [abc]!"); // named argument is not allowed.

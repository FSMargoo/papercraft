...
TEST(codeunit_sequence, concatenate) {
	SCOPED_DETECT_MEMORY_LEAK() {
		...
		const codeunit_sequence combine =
			codeunit_sequence::build("我", cuq_ai, cuq_ai, codeunit_sequence("あなた"), "!"_cuqv);
		...
		{
			{
				...
				EXPECT_EQ(cuq.replace("aabb"_cuqv, "b"_cuqv),
						  "aaaabbaabbaabbaaaabbaabbaabbaaaabbaabbaabbaaaabbaabbaabbaaaabbaabb"_cuqv);
			}
			{
				codeunit_sequence cuq("abbabbabbabbab");
...
TEST(codeunit_sequence, join) {
	SCOPED_DETECT_MEMORY_LEAK() {
		const sequence<codeunit_sequence_view> views	= {"This"_cuqv, "is"_cuqv,	 "a"_cuqv,	 "very"_cuqv,
														   "very"_cuqv, "long"_cuqv, "text"_cuqv};

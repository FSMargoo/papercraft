...
PMCIInstance *PMCIManager::Open(const PString &FilePath, const PString &Alias, const int &X, const int &Y,
								const float &Range) {
	mciSendString(PStringToCStr<TCHAR>(ostr::format("open \"{}\" alias {}", FilePath, Alias)), nullptr, 0, nullptr);
	auto instance	 = new PMCIInstance(Alias);
...
void PMCIManager::PlayAndCloseBGM(const PString &FilePath, const PString &Alias) {
	std::thread(
		[](const PString &FilePath, const PString &Alias) -> void {
...
void PMCIManager::PlayWait(PMCIInstance *Instance, PMCIManagerCameraInterface *Camera) {
	...
	while (now < length) {
		...
		if (!Instance->_isBGM) {
			...
			mciSendString(PStringToCStr<TCHAR>(ostr::format("setaudio {} volume to {}", Instance->_alias, newVolume)),
						  nullptr, 0, nullptr);
		}
